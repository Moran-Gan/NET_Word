 ...
 * @note   Null
 */
unsigned char mcu_get_dp_download_bool(const unsigned char value[])
{
    return(value[0]);
}
 ...
 * @note   Null
 */
unsigned char mcu_get_dp_download_enum(const unsigned char value[])
{
    return(value[0]);
}
 ...
 * @note   Null
 */
unsigned long mcu_get_dp_download_value(const unsigned char value[])
{
...
void wifi_uart_service(void)
{
 //   #error "请直接在main函数的while(1){}中添加wifi_uart_service(),调用该函数不要加任何条件判断,完成后删除该行" 
    ...
    rx_in -= offset;
    if(rx_in > 0) {
        memcpy((char *)wifi_data_process_buf, (const char *)wifi_data_process_buf + offset, rx_in);
    }
}
