...
static void heat_beat_check(void)
{
    ...
    static unsigned char mcu_reset_state = FALSE,mcu_reset_state_bit = 0;
    ...
    if(mcu_reset_state_bit	==	0)
		{
			wifi_uart_write_frame(HEAT_BEAT_CMD, MCU_TX_VER, length);
		}
		else
		{
			wifi_uart_write_frame(0x24, MCU_TX_VER, 0);
		}
		
		mcu_reset_state_bit =	~mcu_reset_state_bit;
...
static void product_info_update(void)
{
    ...
    length = set_wifi_uart_buffer(length, "{\"p\":\"", strlen("{\"p\":\""));
    length = set_wifi_uart_buffer(length,(unsigned char *)PRODUCT_KEY,strlen((unsigned char *)PRODUCT_KEY));
    length = set_wifi_uart_buffer(length, "\",\"v\":\"", strlen("\",\"v\":\""));
    length = set_wifi_uart_buffer(length,(unsigned char *)MCU_VER,strlen((unsigned char *)MCU_VER));
    length = set_wifi_uart_buffer(length, "\",\"m\":", strlen("\",\"m\":"));
    length = set_wifi_uart_buffer(length, (unsigned char *)CONFIG_MODE, strlen((unsigned char *)CONFIG_MODE));
#ifdef CONFIG_MODE_DELAY_TIME
    sprintf((char *)str,",\"mt\":%d",CONFIG_MODE_DELAY_TIME);
    length = set_wifi_uart_buffer(length, str, strlen(str));
#endif
    ...
    length = set_wifi_uart_buffer(length, "}", strlen("}"));
...
static unsigned char data_point_handle(const unsigned char value[])
{
    ...
    if(dp_type != download_cmd[index].dp_type) {
        ...
        ret = dp_download_handle(dp_id,value + 4);
    }
...
void data_handle(unsigned short offset)
{
    ...
    switch(cmd_type)
    {
        ...
        break;
#endif
				case	0x24:																							//获取WiFi信号强度
					result_dbm = wifi_data_process_buf[offset + DATA_START];
					WIFI_dBM	=	result_dbm;
					write_dgus_vp(DISP_wifi_dbm_state,(u8*)&WIFI_dBM,1);				//显示硬件版本号
				break;
